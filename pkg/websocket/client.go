...
// return, so it runs at most once per client today. The sync.Once enforces
// that rather than leaving it to the control flow, so a future call site
// cannot turn this into a close of a closed channel.
